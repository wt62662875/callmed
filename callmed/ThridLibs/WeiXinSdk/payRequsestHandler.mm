...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPay_demo:(AlipayModel *)model
{

    NSString *order_info =model.title;
    if (!order_info) {
        order_info = model.body;
    }
    if (!order_info) {
        order_info =@"保密";
    }
    //订单标题，展示给用户
    NSString *order_name    = order_info;//model.body==nil;
    //订单金额,单位（分）
    NSString *order_price   = model.total_fee;//@"1";//1分钱测试
...
//    NSString *orderno   = [NSString stringWithFormat:@"%ld",time(0)];
    ...
    // 参数相关含义，详见：https://pay.weixin.qq.com/wiki/doc/api/app.php?chapter=9_1
    ...
    [packageParams setObject: model.notify_url        forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: model.out_trade_no           forKey:@"out_trade_no"];//商户订单号
    [packageParams setObject: [WXUtil deviceIPAddress]    forKey:@"spbill_create_ip"];//发器支付的机器ip
    [packageParams setObject: order_price       forKey:@"total_fee"];       //订单金额，单位为分
    [packageParams setObject:model.time_start forKey:@"time_start"]; // 交易起始时间
    [packageParams setObject:model.time_expire forKey:@"time_expire"]; // 交易结束时间
