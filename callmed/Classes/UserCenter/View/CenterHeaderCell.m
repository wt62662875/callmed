...
@property (nonatomic,strong) UIImageView *bt_icon1;
...
- (void) initView
{
    ...
    UIImageView *backImage = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"ding"]];
    ...
    _bt_icon1 = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"icon_userinfo"]];
    [self addSubview:_bt_icon1];
    ...
    [_lb_name_v setFont:[UIFont systemFontOfSize:17]];
...
- (void) setModel:(BaseItemModel *)model
{
    ...
    if ([GlobalData sharedInstance].user.isLogin) {
        ...
        [_lb_name_v setText:[GlobalData sharedInstance].user.userInfo.realName];
    ...
    if ([GlobalData sharedInstance].user.isLogin) {
        [_bt_icon1 mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(_bt_login_v);
            make.rightMargin.equalTo(_bt_login_v.mas_left);
        }];
    }
}
