...
+(void)showTimePickerViewDeadLine:(NSString *)deadLine CompleteBlock:(void (^)(NSDictionary *))completeBlock{
    ...
    timePickerView.completeBlock =^(NSDictionary *infoDic) {
        [[UIApplication sharedApplication].keyWindow setAlpha:1.0];
        if (completeBlock) {
            completeBlock(infoDic);
        }
    };
    ...
    [[UIApplication sharedApplication].keyWindow setAlpha:0.8];
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        timePickerView.maskView.alpha = 1.0;
