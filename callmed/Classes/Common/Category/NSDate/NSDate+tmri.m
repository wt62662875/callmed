...
-(NSString *)dateToDetailStr
{
    return [self dateToStrByFormat:@"yyyy-MM-dd HH:mm:ss"];
}
...
//返回相差天数对应的date
-(NSDate *)dayByInt:(int)day
{
    NSDateComponents *components = [[NSDateComponents alloc] init];
    [components setDay:day];
    return [[[self class] sharedCalendar] dateByAddingComponents:components toDate:self options:0];
}
...
-(NSString *)dateToDate:(NSDate *)date
{
    ...
    if (diff.day > 0) {
        return [NSString stringWithFormat:@"%ld天%ld时",(long)diff.day,(long)diff.hour];
    }else if(diff.hour >0 )
    {
        return [NSString stringWithFormat:@"%ld时%ld分",(long)diff.hour,(long)diff.minute];
    }else if(diff.minute >0)
    {
        return [NSString stringWithFormat:@"%ld分%ld秒",(long)diff.minute,(long)diff.second];
    }else
    {
