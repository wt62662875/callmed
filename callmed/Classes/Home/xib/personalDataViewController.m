...
@interface personalDataViewController ()<UITableViewDataSource,UITableViewDelegate,TargetActionDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UIActionSheetDelegate,getCompanyDelegate,AMapLocationManagerDelegate>
{
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setTitle:@"个人资料"];
    self.view.backgroundColor = RGBHex(g_red);
    ...
    [loginOut setTitle:@"确定修改" forState:UIControlStateNormal];
    [loginOut setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [loginOut setBackgroundColor:RGBHex(g_red)];
    [loginOut addTarget:self action:@selector(buttonTarget:) forControlEvents:UIControlEventTouchUpInside];
